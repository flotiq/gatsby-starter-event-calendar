...
exports.importExamples = async () => {
    ...
    let contentTypeDefinitionResponse = await fetch(apiUrl + `/api/v1/internal/contenttype/event`, { headers: headers })
    let imageToReplace = []
    ...
    await Promise.all(files.map(async function (file) {
        if (file.indexOf(`contentObject`) === 0) {
            ...
            let res = await fetch(apiUrl + `/api/v1/content/event/` + id, { method: `HEAD`, headers: headers })
            let method = `POST`
            let url = apiUrl + `/api/v1/content/event`
            if (res.status === 403) {
                assert.ok(false, `Please provide correct API key (Read and write Application API key)`)
            }
            if (res.ok) {
            ...
            imageToReplace.forEach((image, index) => {
                ...
                contentObject = contentObject.replace(regex, imageForReplacing[index])
            })
            res = await fetch(url, {
                method: method,
                body: contentObject,
